<template>
  <div class="bg-white rounded border border-gray-200 relative flex flex-col">
    <div class="px-6 pt-6 pb-5 font-bold border-b border-gray-200">
      ...
      <em class="fas fa-upload float-right text-green-400 text-2xl"></em>
    </div>
    <div class="p-6">
      <!-- Upload Dropbox -->
      <div
      ...
      <label for="file_upload_input">Choose files ->
        <input id="file_upload_input" multiple type="file" @change="upload($event)"/>
      </label>
      <hr class="my-6"/>
      <!-- Progress Bars -->
      <div v-for="upload in uploads" :key="upload.name" class="mb-4">
        ...
        <div :class="upload.text_class" class="font-bold text-sm">
          <em :class="upload.icon"></em> {{ upload.name }}
        </div>
        <div class="flex h-4 overflow-hidden bg-gray-200 rounded">
          ...
          <div
            ...
            class="transition-all progress-bar"></div>
        </div>
      </div>
    </div>
  </div>
